        ...
        public override void Running(RunningModes runningMode)
        {
            ...
            while (true)
            {
                ...
                if (stationOperate.Running)
                {
                    ...
                    {
                        case 0://判断是否有料
                            if (Marking.RightCut2HaveProduct) step = 210;
                            else step = 10;
                            break;
                        case 10://前后气缸为ON
                            if (DownCylinder.OutOriginStatus&&FrontCylinder.OutOriginStatus&&GripperCylinder.OutOriginStatus
                                && FrontCylinder.Condition.IsOnCondition)
                            {
                                FrontCylinder.Set();
                                step = 20;
                            }
                            break;
                        case 20://判断蜂窝该穴位是否屏蔽
                            if (FrontCylinder.OutMoveStatus&& (Marking.RightCAxis2Finish || stationOperate.SingleRunning))
                            {
                        ...
                        case 40://夹子气缸到位,伺服2#移动到剪切，启动运行
                            ...
                            {
                                CutAxis.MoveTo(Position.RightCut2CutPosition, AxisParameter.RightCut2velocityCure);
                                step = 50;
                            }
                            break;
                        case 50://伺服1#到位，判断是否扭力剪切
                            if (CutAxis.IsInPosition(Position.RightCut2CutPosition))
                            {
                                Thread.Sleep(20);
                        ...
                        case 60://启动伺服1#扭力剪切控制
                            ...
                            break;
                        case 70://伺服1#移动0位置。
                            CutAxis.MoveTo(0, AxisParameter.RightCut2velocityCure);
                            step = 80;
                            break;
                        case 80://伺服1#到位。
                            if (CutAxis.IsInPosition(0))
                            {
                                Product.RightCut2Count++;
                                Thread.Sleep(20);
                                step = 90;
                            }
                            break;
                        case 90://判断剪切前后气缸off条件
                            ...
                            {
                                Product.Right2ProductTotal++;
                                Marking.RightCut2Num = Marking.RightCcellNum;
                                FrontCylinder.Reset();
                                step = 100;
                            }
                            break;
                        ...
                        case 140://判断切料准备好，吸笔吸气标志状态，夹子气缸为OFF
                            if ((Marking.XYZRightInhale2Sign || stationOperate.SingleRunning)
                        ...
                        case 150://夹子气缸到位，切料准备状态复位
                            if (GripperCylinder.OutOriginStatus)
                            {
                                ...
                                if (Marking.RightCut2Num >= Position.TrayCellNum && Marking.ForceInPlate) Marking.RightCut2Done = true;
                                step = 160;
                            }
                            break;
                        case 160://判断吸笔吸气状态
                            if ((!Marking.RightCut2Finish || stationOperate.SingleRunning)
                ...
                if (stationInitialize.Running)
                {
                    switch (stationInitialize.Flow)
                    {
                        ...
                        case 40:
                            ...
                            {
                                _isOrigin1 = true;
                                CutAxis.MoveDelta(10000, AxisParameter.RightCut2velocityCure);
                            }
