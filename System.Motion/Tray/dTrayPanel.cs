...
{
    public partial class dTrayPanel : UserControl
    {
        ...
        int Col = 3;
        Tray t = null;
        public void SetTrayObj(Tray tray,Color initColor,int[] names)
        {
            t = tray;
            Row = t.Row;
            Col = t.Column;
            SetLayout(initColor, names);
        }
        ...
        private void DynamicLayout(TableLayoutPanel layoutPanel,Color bColor, int[] names)
        {
            ...
            for (int i = 0; i < Row; i++)
            {
                layoutPanel.RowStyles.Add(new RowStyle(SizeType.Absolute, 33));
            }
            layoutPanel.ColumnCount = Col+1;    //设置分成几列  
            for (int i = 0; i < Col; i++)
            {
                layoutPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 33));
            }
            for (int i = 0; i < Row; i++)
            {
                ...
                for (int j = 0; j < Col; j++)
                {
                    ...
                    btn.Font=new Font("宋体", 15.0F, FontStyle.Regular, GraphicsUnit.Point, ((byte)(134)));
        ...
        public void SetLayout(Color bColor, int[] names)
        {
            t.SortTray(t.StartPose,t.Direction,t.ChangeLineType);
            DynamicLayout(tableLayoutPanel1, bColor, names);
        }
        ...
        public dCoordinate Locate { get;private set; }
