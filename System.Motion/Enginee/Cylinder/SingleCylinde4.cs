...
namespace System.Enginee
{
    ...
    /// 三气缸共用电磁阀控制块
    /// </summary>
    public class SingleCylinder4 : Cylinder, ICylinderStatusJugger
    {
        #region "字段"
        ...
        private IoPoint _InOrigin1, _InOrigin2, _InOrigin3, _InOrigin4, _InMove1, _InMove2, _InMove3, _InMove4, _OutMove;
        ...
        public bool _NoMove1 = false; //1#屏蔽动点
        #endregion
        public SingleCylinder4(IoPoint InOrigin1, IoPoint InOrigin2, IoPoint InOrigin3, IoPoint InOrigin4, IoPoint InMove1, IoPoint InMove2, IoPoint InMove3, IoPoint InMove4, IoPoint OutMove)
        {
            _InOrigin1 = InOrigin1;
            _InOrigin2 = InOrigin2;
            _InOrigin3 = InOrigin3;
            _InOrigin4 = InOrigin4;
            _InMove1 = InMove1;
            _InMove2 = InMove2;
            _InMove3 = InMove3;
            _InMove4 = InMove4;
        ...
        private bool NoOriginAlarm()
        {
            ...
            if (Condition.External.AirSignal && !_OutMove.Value && (!_InOrigin1.Value || !_InOrigin2.Value || !_InOrigin3.Value || !_InOrigin4.Value)) cylinderNoOrigin = true;
        ...
        private bool NoMoveAlarm()
        {
            ...
            if(_NoMove1)
            {
                if (Condition.External.AirSignal && _OutMove.Value && (!_InMove2.Value || !_InMove3.Value || !_InMove4.Value)) cylinderNoMove = true;
                else cylinderNoMove = false;
            }
            else
            {
                if (Condition.External.AirSignal && _OutMove.Value && (!_InMove1.Value || !_InMove2.Value || !_InMove3.Value || !_InMove4.Value)) cylinderNoMove = true;
                else cylinderNoMove = false;
            }
        ...
        private bool OffMoveAlarm()
        {
            ...
            if(_NoMove1)
            {
                if (Condition.External.AirSignal && !_OutMove.Value && _InMove2.Value && _InMove3.Value && _InMove4.Value) cylinderOffMove = true;
                else cylinderOffMove = false;
            }
            else
            {
                if (Condition.External.AirSignal && !_OutMove.Value && _InMove1.Value && _InMove2.Value && _InMove3.Value && _InMove4.Value) cylinderOffMove = true;
                else cylinderOffMove = false;
            }
        ...
        {
            bool alarmDelayDone = false;
            if (Condition.External.AirSignal && _OutMove.Value && _InOrigin1.Value && _InOrigin2.Value && _InOrigin3.Value && _InOrigin4.Value) cylinderOnOrigin = true;
        ...
        {
            //原点状态输出
            if (!_OutMove.Value && ((_InOrigin1.Value && _InOrigin2.Value && _InOrigin3.Value && _InOrigin4.Value && !Condition.NoOriginShield)
            ...
            if(_NoMove1)
            {
                if (_OutMove.Value && ((_InMove2.Value && _InMove3.Value && _InMove4.Value && !Condition.NoMoveShield)
            ...
            {
                if (_OutMove.Value && ((_InMove1.Value && _InMove2.Value && _InMove3.Value && _InMove4.Value && !Condition.NoMoveShield)
                    || Condition.NoMoveShield))
                {
                    _watchMove.Stop();
                    if (Delay.MoveTime <= _watchMove.ElapsedMilliseconds) _OutMoveStatus = true;
